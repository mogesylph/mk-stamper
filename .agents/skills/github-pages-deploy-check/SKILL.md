...
description: GitHub Pages 公開前後の確認をするときに使う。Vite の base、public 配下ファイルの参照パス、GitHub Actions workflow、ビルド結果の asset path を確認し、mk-stamper が https://mogesylph.github.io/mk-stamper/ で壊れず動くかをチェックする。通常の UI 変更や機能実装だけの場合には使わない。
---
...
mk-stamper は `https://mogesylph.github.io/mk-stamper/` で公開するため、ルート `/` ではなく `/mk-stamper/` 配下で動く前提で確認する。
...
- 公開先 URL は `https://mogesylph.github.io/mk-stamper/` として扱う
- Vite の `base` は `/mk-stamper/` を前提にする
...
ビルド後に `app/dist/index.html` を確認し、JS / CSS / favicon の参照が `/mk-stamper/` から始まっているか見る。

必要に応じて次を実行する。

```sh
rg -n "/mk-stamper/|/songlist.tsv" app/dist
```
